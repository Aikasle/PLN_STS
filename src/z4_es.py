...
def main():
    ...
    batch_size = 16
    epochs = 4
    model_save_path = './output/z4_finetuned_es'

    print(f">>> Iniciando Z4: Fine-Tuning Directo en Español sobre {model_name}")
    ...
    for row in dataset['train']:
        train_examples.append(InputExample(
            ...
            label=row['similarity_score'] / 5.0  
        ))
    ...
    evaluator = EmbeddingSimilarityEvaluator(
        val_s1, val_s2, val_scores, name='sts-val-es'
    )

    train_loss = losses.CosineSimilarityLoss(model)

    print(f">>> Entrenando el modelo {model_name} con datos en ESPAÑOL...")
    ...
    print(f"Pearson Final (Fine-tuned ES): {pearson_final:.4f}")
